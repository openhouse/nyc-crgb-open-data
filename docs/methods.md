...
- **Data source:** Aggregated Local Law 157 storefront registration statistics published by NYC Department of Finance on NYC Open Data:
  ...
  Snapshots are cached under `data-raw/storefront/` via `read_storefront_stats()`, refreshed when `CRGB_REFRESH_OPEN_DATA=true`, and can fall back to local fixtures when `CRGB_USE_OPEN_DATA=false`.
- **Column mapping:**
  - `reporting_year` → `year`
  - `aggregate_level_citywide` → `geography_type` (lower case) and `geography_name` (title case)
  ...
  - `storefront_reported_not_leased` → `vacant_storefronts`; `vacancy_rate` computed as `vacant_storefronts / total_storefronts`
